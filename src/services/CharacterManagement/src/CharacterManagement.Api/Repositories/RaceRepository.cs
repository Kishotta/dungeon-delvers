...
public class RaceRepository(CharacterManagementContext context)
{
    ...
    {
        return await context.Races.Where (s => s.OwnerId == userId).ToListAsync(cancellationToken);
    }

    public async Task<Race?> GetByIdAsync (Guid id, CancellationToken cancellationToken)
    {
        return await context.Races.FirstOrDefaultAsync (s => s.Id == id, cancellationToken);
    }
